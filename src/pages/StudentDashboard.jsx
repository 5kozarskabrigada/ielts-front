...
/**
 * Unified display text resolver for all question types.
 * Works for both UI rendering and PDF generation.
 *
 * @param {object} ans  - answer object (question_type, question_text, question_template, label_text)
 * @param {number} blankIndex - 0-based blank index within a template group (from templateToBlankIndex map)
 * @returns {string}
 */
function getQuestionDisplayText(ans, blankIndex = 0) {
  const qt = String(ans.question_type || '').toLowerCase();
  const GENERIC_RE = /^(summary|sentence|table|form|note|diagram|map|short_answer|[a-z_]+)\s+(blank|item|completion|labeling)\s+\d+$/i;
  ...
  if (qt === 'table_completion') {
    const label = String(ans.label_text || '').trim();
    const tmpl  = String(ans.question_template || '').trim();
    if (tmpl) {
      const extracted = extractSentenceForBlank(tmpl, blankIndex);
      const isBareBlank = !extracted || extracted.replace(/_/g, '').trim() === '';
      if (label) return `${label}: ${isBareBlank ? '___' : extracted}`;
      return isBareBlank ? (label ? `${label}: ___` : `Question ${ans.question_number}`) : extracted;
    }
    if (label) return `${label}: ___`;
    // No template and no label — try question_text
    const qText = String(ans.question_text || '').trim();
    if (qText && !GENERIC_RE.test(qText)) return qText;
  ...
  // ── form_completion ───────────────────────────────────────────────────────
  if (qt === 'form_completion') {
    if (ans.question_template) {
      return String(ans.question_template).trim().replace(/\[BLANK\]/g, '___');
    }
  }

  // ── all other template-based types (summary, sentence, note, …) ──────────
  const templateTypes = ['summary_completion', 'sentence_completion', 'note_completion', 'form_completion'];
  if (templateTypes.includes(qt)) {
    if (ans.question_template && String(ans.question_template).trim()) {
      return extractSentenceForBlank(ans.question_template, blankIndex);
    }
  }
        ...
        sections.forEach(([sectionTitle, data]) => {
          ...
          const body = sortedRows
            .map((a) => {
              ...
              // Resolve question display text using the shared helper
              const blankIndex = templateToBlankIndex.get(a.question_number) || 0;
              const displayText = getQuestionDisplayText(a, blankIndex) ||
                `[Question ${a.question_number}: text not available]`;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center space-x-3">
          <button
            onClick={() => { setLoading(true); fetchSubmissionDetails(); }}
            className="flex items-center space-x-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition text-sm font-medium pdf-hide"
            title="Reload question text from database"
          >
            <RefreshCw size={16} />
                            ...
                            >
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1" style={{minWidth: 0, maxWidth: '100%'}}>
                                  {/* Question Text */}
                                  {(() => {
                                    // Resolve display text using the shared helper
                                    const blankIndex = templateToBlankIndex.get(ans.question_number) || 0;
                                    const displayText = getQuestionDisplayText(ans, blankIndex);
