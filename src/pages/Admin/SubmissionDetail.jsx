        ...
        sections.forEach(([sectionTitle, data]) => {
          ...
          const body = sortedRows
            .map((a) => {
              ...
              if (isTemplateType) {
                // map_labeling / diagram_labeling: question_text IS the question label — use it directly
                if (a.question_type === 'map_labeling' || a.question_type === 'diagram_labeling') {
                  const qText = String(a.question_text || '').trim();
                  if (qText && !qText.match(/^(Map|Diagram)\s+(blank|labeling|completion)\s+\d+$/i)) {
                    displayText = qText;
                  }
                }

                // table_completion: combine label_text (left col) with question_template (right col)
                if (!displayText && a.question_type === 'table_completion') {
                  const label = String(a.label_text || '').trim();
                  const tmpl = String(a.question_template || '').trim();
                  if (tmpl) {
                    const blankIndex = templateToBlankIndex.get(a.question_number) || 0;
                    const extracted = extractSentenceForBlank(tmpl, blankIndex);
                    // If extracted is just "___" or empty, prepend the label for context
                    const isBareBlank = !extracted || extracted.replace(/_/g, '').trim() === '';
                    displayText = label
                      ? `${label}: ${isBareBlank ? '___' : extracted}`
                      : (isBareBlank ? '' : extracted);
                  } else if (a.label_text) {
                    // No template at all — just show the label
                    displayText = String(a.label_text).trim() + ': ___';
                  }
                }

                // For form_completion, just use the template (don't combine with label)
                if (!displayText && a.question_type === 'form_completion' && a.question_template) {
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1" style={{minWidth: 0, maxWidth: '100%'}}>
                                  ...
                                  {(() => {
                                    ...
                                    let displayText = '';
                                    if (isTemplateType) {
                                      // map_labeling / diagram_labeling: question_text IS the label — use it directly
                                      ...
                                      // table_completion: combine label_text (left col) + question_template (right col)
                                      if (!displayText && ans.question_type === 'table_completion') {
                                        ...
                                        if (tmpl) {
                                          ...
                                          displayText = label
                                            ? `${label}: ${isBareBlank ? '___' : extracted}`
                                            : (isBareBlank ? '' : extracted);
                                        } else if (ans.label_text) {
                                          displayText = String(ans.label_text).trim() + ': ___';
                                        }
                                      }

                                      // For form_completion, just use the template (don't combine with label)
                                      // The template should already contain the full question text
                                      if (!displayText && ans.question_type === 'form_completion' && ans.question_template) {
